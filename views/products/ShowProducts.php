<div class="container">
    ...
    while ($array = $show->fetchAll(PDO::FETCH_ASSOC)) {
        foreach($array as $data) {
            ...
            echo "<td><a href= 'index.php?controller=category&action=getCategoriesEdit&id=".$data['IdProducto']."&name=".$data['Nombre']."&desc=".$data['Descripcion']."&category=".$data['Genero']."&author=".$data['Autor']."&editorial=".$data['Editorial']."&pages=".$data['Paginas']."&price=".$data['Precio']."&date=".$data['Fecha_Entrada']."&date=".$data['Fecha_Entrada']."&quantity=".$data['Cantidad']."'> <img src='views/img/lapiz.png' width='60'> </a></td>";
